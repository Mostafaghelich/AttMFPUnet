...
class ISO_valid(object):
    ...
    def transform(self, X, y=None):
        if y is None:
            ...
            X = np.expand_dims(X, axis=self.axis) 
            y = np.expand_dims(y, axis=self.axis)

        p = np.reshape(np.array(X), [544,800,1]) > (np.mean(np.array(X))+2*np.std(np.array(X)))
        X_prime = p*np.reshape(np.array(X),[544,800,1])
